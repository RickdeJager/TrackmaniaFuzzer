...
pub fn get_trackmania_context(tree_depth: usize) -> NautilusContext {
    ...
    // Strings
    // Format string special chars
    for elem in ["%", "%999999s", "%99 ", "%0999d"] {
        add_rule!("STRING", elem.to_string());
    }
    ...
    for c in "\t\r\n\0abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$^&*()_+'<>?[]|,".chars() {
        add_rule!("STRING", c.to_string());
    }
    add_rule!("STRING", "{STRING}{STRING}");

    // Base64
    add_rule!("BASE64", "SGF2ZSBzb21lIGV4YW1wbGUgYmFzZSA2NCEh");
    ...
    // Doubles
    add_rule!("DOUBLE", "{NATURAL_NUMBER}.{NATURAL_NUMBER}");
    add_rule!("DOUBLE", "-{NATURAL_NUMBER}.{NATURAL_NUMBER}");
    add_rule!("DOUBLE", "{INT}");
    add_rule!("DOUBLE", "1.0");
    add_rule!("DOUBLE", "0.0");
